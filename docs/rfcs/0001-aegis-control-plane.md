...
|---|---|---|---|---|
| **Custom Aegis service** | Typed control API, versioned flows, SQLite + JSONL, Herdr adapters, TUI and Hermes client | Engineering and maintenance time; low steady-state runtime | New local component built around existing repo policy | Repository license |
...
Mastra has useful workflow primitives, but Aegis still needs custom capability,
...
flows and Mastra's persisted suspend/resume are credible options if Aegis later
...
- **Exposure (§5):** no public listener. Aegis uses a Unix socket under
  `/run/aegis`; the TUI is reached over existing SSH/Tailscale and Telegram
  reaches it through the restricted Hermes gateway.
- **Secrets owner (§4):** `agentops` owns Aegis and long-lived adapter secrets.
...
Aegis is built in this repository, installed as a pinned Python package under
`agentops`, and started by a hardened systemd unit. Provisioning creates
`hermesops` and `agentops`, a rootless worker runtime, `/run/aegis` socket
...
Build Aegis as a deterministic Python 3.12 service. Keep `dev` unchanged during
