...
Date: 2026-07-23
Sponsor: Harness — needs a configurable, provider-agnostic implementation runtime
...
able to use cheaper model aliases without changing Harness flows. The runtime
...
| Direct custom model worker | Harness implements its own tool loop | Highest security/control and implementation burden | New | Local code |
...
- **Secrets owner (§4):** upstream provider keys stay with `agentops`'s Harness
...
Harness role with exact `allow`/`ask`/`deny` permissions; Harness's container and
...
default, task-scoped worktree mounts, role-specific skills, and a Harness model
