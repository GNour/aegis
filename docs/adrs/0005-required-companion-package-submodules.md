...
provides a broad role, skill, and handoff catalog. Harness benefits from both, and
...
Harness policy, skill isolation, and reproducibility.

## Decision

Harness will track PromptX and Subagents as pinned Git submodules under
`packages/` using HTTPS fetch URLs.

PromptX is a required runtime dependency of the Harness control plane and is
called through a fixed typed adapter. Subagents is a required build-time catalog
dependency whose validated, compiled output is embedded in Harness releases.
...
Harness maintainers actively maintain and enhance both companion repositories.
Companion changes land on their own branches and in their own history before a
separate Harness commit advances the pinned submodule pointer. Harness release
...
reproducible artifacts. Harness must maintain catalog mappings, adapter contract
...
- Vendor snapshots into Harness — simple builds, but duplicated ownership and
  upstream drift.
