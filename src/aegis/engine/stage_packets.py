...
Pure: no I/O, no clock, no global configuration. The canonical hash is computed over the
compact, sorted JSON of the packet with the hash field blanked, so it is stable across
processes and independent of key ordering. Round-tripping goes through JSON (not Python
dicts) so the strict domain models accept arrays as tuples.
"""
