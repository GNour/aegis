...
def test_migration_is_recorded_once_after_reopening(tmp_path) -> None:
    ...
    expected = (
        "0001_initial",
        "0002_audit_outbox_leases",
        "0003_stage_execution_packets",
        "0004_control_api",
    )
...
def test_migrations_upgrade_and_bootstrap_with_a_future_schema_change(tmp_path) -> None:
    ...
    (migrations / "0004_control_api.sql").write_text(
        (sqlite_module._SCHEMA_DIR / "0004_control_api.sql").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    base = (
        "0001_initial",
...
"""
    ...
    (migrations / "0005_upgrade.sql").write_text(upgrade, encoding="utf-8")
    with SQLiteStore(upgraded_path, schema_dir=migrations, allow_schema_extensions=True) as store:
        assert store.applied_migrations() == (*base, "0005_upgrade")
...
def test_migration_rejects_form_feed_prefixed_transaction_control(tmp_path) -> None:
    migrations = tmp_path / "migrations"
    migrations.mkdir()
    (migrations / "0001_initial.sql").write_text(
        (sqlite_module._SCHEMA_DIR / "0001_initial.sql").read_text(encoding="utf-8"), encoding="utf-8"
    )
    (migrations / "0002_bad.sql").write_text("\fCOMMIT;", encoding="utf-8")
